...
library(tidyverse)
library(here)
load(here("data/presidential_precincts_2016.rda"))
```
...
county_tots <- function(state_abbrv, df=df1, csv="Y") {
  ...
  tot_state %>%
    mutate(totals = rowSums(.[4:dim(.)[2]], na.rm = T)) %>%
    select(county_name, county_fips, precinct, totals) %>%
  ...
  tot_state %>%
    ...
    arrange(county_name) -> county_tots
  return(county_tots)
  
  if (csv=="Y") {
  write.csv(as.data.frame(county_tots), here(paste0('/',state_abbrv,'-county_tots.csv')),
            row.names = F)
...
modes_by_county <- function(state_abbrv, csv="Y", prop="N") {
    ...
    if (prop=="Y") {
      ...
      write.csv(as.data.frame(sweep(df_transpose, 1, as.matrix(county_tots[,3]), FUN = '/')), 
